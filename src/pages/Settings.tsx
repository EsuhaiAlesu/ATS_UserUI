...
const INPUT = 'w-full bg-surface text-on-surface border border-outline-variant rounded py-2 px-3 text-sm focus:ring-0 focus:border-secondary field-lux transition-shadow';
            ...
            <div className="flex-1 overflow-y-auto">
                <main className="max-w-2xl mx-auto px-6 py-8 space-y-5">
                    ...
                    <Section id="pd" icon="format_size" title="Hiển thị phụ đề" desc="Cỡ chữ mặc định cho Tường phụ đề (trước đây chỉ chỉnh được bằng phím +/−).">
                        ...
                        <div className="border border-outline-variant rounded bg-surface-container-lowest px-4 py-3 overflow-hidden">
                    ...
                    <Section id="gd" icon="record_voice_over" title="Giọng đọc (TTS)" desc="Máy đọc thành tiếng, hay chỉ hiện phụ đề.">
                        ...
                        <div className="text-sm text-on-surface-variant">Giọng đang chọn — Vietnamese: <span className="text-on-surface">{ttsPrefs.vi?.label ?? '—'}</span> · 日本語: <span className="text-on-surface">{ttsPrefs.ja?.label ?? '—'}</span></div>
