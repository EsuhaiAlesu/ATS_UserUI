...
- **2026-07-19 (tối)** — ✅ **IA Phase 1 (khung điều hướng) ĐÃ BUILD** (`OperatorLayout` — 1 rail trái theo vòng đời + khối An toàn ghim, /prep là nhà, /→/prep, bỏ 3 nav cũ + sidebar /audio + back-link, xóa Home/MainLayout chết). Verify trình duyệt: rail nhất quán, active theo route, /stream+/reveal toàn màn hình, EMERGENCY STOP mọi trang, 0 lỗi console. *Chờ Railway Redeploy để lên live.* Kế: Phase 2 (gọn /audio SETUP↔RUN).
