...
- **2026-07-19 (khuya)** — ✅ **Hoàn thiện WOW — 3 hạng mục còn lại XONG** (commit `bd67d4c`, đẩy develop): (1) **Accessibility** — vòng focus-visible **vàng** toàn cục (`index.css`, đã verify hiện trên nav khi Tab), icon trang trí gắn `aria-hidden`, mọi nút thao tác đều có nhãn chữ; (2) **Thống nhất icon** — thay emoji màu `🔒/🔊/⚠` (render màu OS, phá đen–vàng) bằng Material Symbols `lock/volume_up/warning` (tint theo token), pre-flight `✓/✕`→`check_circle/cancel`, bỏ `●` thừa ở nhãn LIVE; (3) **Phase 2 `/audio` gọn** — pre-flight gom mục ĐẠT thành 1 dòng "N/7 mục đã đạt", chỉ liệt kê mục CHƯA đạt (hết bức tường ✕ đỏ khi offline). Build+oxlint sạch, 0 lỗi console. *Chờ Railway Redeploy để lên live.*
